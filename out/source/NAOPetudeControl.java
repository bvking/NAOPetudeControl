/* autogenerated by Processing revision 1286 on 2023-03-28 */
...
public class NAOPetudeControl extends PApplet {
...
// int splitTimeLfo= int (timeLfo);
 ...
 //   key= 'j'; keyPressed (); // to start Live 
    ...
    for (int i = 0; i < networkSize; i++) {
      ...
      if (netPhaseBase[i] >  0 ) {  
        CircularOldVirtualPosition[i]=CircularVirtualPosition[i]; 
        CircularVirtualPosition[i]= PApplet.parseInt (map (netPhaseBase[i], 0, TWO_PI, 0-1600, numberOfStep-1600));
     ...
     if ((CircularVirtualPosition[i]>3199 && CircularOldVirtualPosition[i]<3200 && CircularOldVirtualPosition[i]>201  )
       ...
       print (i);  print(" CIRCULAR PASS CLOCKWISE THROUG 0: "); println (  TrigmodPos[i]=0); print (" virt ");  println (  VirtualPosition[i]); print (" Cirvirt "); print(  CircularVirtualPosition[i]); print (" CirOldvirt "); println (  CircularOldVirtualPosition[i]);
        ...
        CircularOldVirtualPosition[i]=CircularVirtualPosition[i]; 
        CircularVirtualPosition[i]= PApplet.parseInt (map (netPhaseBase[i], 0, -TWO_PI, numberOfStep-1600, 0-1600));  
    ...
    for (int i = 0; i < networkSize; i++) {
      ...
      if (net.phase[i] >  0 ) {  
        CircularOldVirtualPosition[i]=CircularVirtualPosition[i]; 
        CircularVirtualPosition[i]= PApplet.parseInt (map (net.phase[i], 0, TWO_PI, numberOfStep/2, numberOfStep+numberOfStep/2));
        Pos[i]= PApplet.parseInt (map (net.phase[i], 0, TWO_PI, 0, 127)); // to Oscsend
     ...
     if ((CircularVirtualPosition[i]>3199 && CircularOldVirtualPosition[i]<3200 && CircularOldVirtualPosition[i]>201  )
       ...
       print (i);  print(" CIRCULAR PASS CLOCKWISE THROUG 0: "); println (  TrigmodPos[i]=0); print (" virt ");  println (  VirtualPosition[i]); print (" Cirvirt "); print(  CircularVirtualPosition[i]); print (" CirOldvirt "); println (  CircularOldVirtualPosition[i]);
        ...
        CircularOldVirtualPosition[i]=CircularVirtualPosition[i]; 
        CircularVirtualPosition[i]= PApplet.parseInt (map (net.phase[i], 0, -TWO_PI, numberOfStep+numberOfStep/2, numberOfStep/2));  
        Pos[i]= PApplet.parseInt (map (net.phase[i], 0, -TWO_PI, 127, 0));  // to Oscsend  
...
//      VirtualPosition[i]= CircularVirtualPosition[i]+ActualVirtualPosition[i]; 
    ...
    ActualVirtualPositionFromOtherMode[i]=(float) VirtualPosition[i];
       ...
       text (PApplet.parseChar (formerKeyMetro) + " circular " + VirtualPosition[i], - 500, -900+100*i);
      }
  ...
  if (formerKeyMetro == '$'|| formerKeyMetro == 'à') {
   ...
   //      PendularVirtualPosition[i]= int (map (metroPhase[i], -PI/2, PI/2,  -numberOfStep/2, 0)); // better : Metronome pass throug position 0
       ...
       PendularVirtualPosition[i]= PApplet.parseInt (map (metroPhase[i], -PI/2, PI/2, 0,  numberOfStep/2)); // better : Metronome pass throug position 0
  ...
  //    PendularVirtualPosition[i]= int (map (metroPhase[i], -0.5*PI, 1.5*TWO_PI, 0, numberOfStep/2)); // movement with more amplitude?
   //   PendularVirtualPosition[i]= (int) map ( PendularVirtualPosition[i], -4800, -1600, -800, 800); // mapped at the scale in Max 4 live
       Pos[i]= PApplet.parseInt (map (PendularVirtualPosition[i], 1600, numberOfStep/2, 0, 127)); // to Oscsend 
    ...
    //  DataToDueCircularVirtualPosition[i]=PendularVirtualPosition[i];//+VirtualPosition[i]
     //  DataToDueCircularVirtualPosition[i]=VirtualPosition[i];
      ...
      text (" net.phase[i] " + net.phase[i] + " " + PApplet.parseChar (formerKeyMetro)  + " pendular " + VirtualPosition[i], - 500, -900+100*i);
...
/*
 ...
 public void followDistribueAddLfoPatternBase(){ 
...
class Sampler {
  ...
  Sampler() {
   ...
   public void addSample( float x, float y ) {  // add sample when bRecording
    int now = millis();
...
//    fill( mapAccelerationinversed[i], 255, 0 ); // Sepheres are all modulated with the same color. depending of acceleration
   ...
   // metroPhase[i]= metroPhase[i];     
    ...
    println ( " metro " + metroPhase[i]  + " formerKeyMetro " + " " + i + " " + PApplet.parseChar (formerKeyMetro) ); 
    ...
    sphereDetail( 4*5);
     ...
     if (formerKeyMetro !='B' ) { //drive ball with lfo
         ...
         println ( net.phase[i]  + " formerKeyMetro " + " " + i + " " + PApplet.parseChar (formerKeyMetro) ); 
      x = displacement*cos(net.phase[i]);
      y = displacement*sin(net.phase[i]);
...
//  newPosF[networkSize-1]= (float) map (mouseY, 0, 400, 0, TWO_PI);
 ...
 public void serialEvent(Serial encoderReceiveUSBport14101) { // receive 2 datas splited with , and the last is send with println
...
//   key = 'q' ;  // propagation in on the same way
  ...
  */
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--full-screen", "--bgcolor=#666666", "--stop-color=#cccccc", "NAOPetudeControl" };
    if (passedArgs != null) {
